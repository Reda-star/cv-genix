...
import LoadingSkillsGeneration from "../components/LoadingSkillsGeneration";

// eslint-disable-next-line react/prop-types, no-unused-vars
const SkillSection = ({ setSkills, title, works  }) => {
  ...
  const handleAddSkill = () => {
    if (selectedSkill || yearsOfExperience) {
      const newSkills = [
        { id: Math.random(), skill: selectedSkill, year: yearsOfExperience },
        ...skills,
      ];
      setSkillsLocal(newSkills);
      setSkills(newSkills); // Update the skills in the parent component
  ...
  useEffect(() => {
    if (selectedSkill && autoAdd) {
      handleAddSkill({ id: Math.random(), name: selectedSkill });
    }
  }, [selectedSkill, autoAdd]);
  ...
  const generateSkills = async () => {
    ...
    try {
      ...
      const worksCompany = works.map(work => work.companyName).join(', ');

      const worksPosition = works.map(work => work.position).join(', ');

      const worksDesc = works.map(work => work.workDescription).join(', ');


      const prompt = `you have the ${title}, ${worksPosition}, ${worksCompany}, ${worksDesc} and the ${skills}, Now generate a list of 6 skills that are compatible to these details and completly different from the skills that you have, with each skill in one word (or two words max)`;
      console.log(prompt);


      const response = await axios.post("http://localhost:7000/skills", {
        prompt: prompt,
      });
      const { data } = response;
      // Loop through each skill received in the 
      setAutoAdd(true);
      for (const skill of data.generatedSkills) {
        setSelectedSkill(skill);
        await new Promise(r => setTimeout(r, 1000)); // wait for a bit // wait for a bit
      }
      setAutoAdd(false);
    } catch (error) {
    ...
    <>
      <div className="p-4">
        <h2 className="text-lg font-semibold mb-4 flex gap-x-2">
          <svg
            className="fill-current text-blue-700 ml-1"
            height="25"
            width="25"
            version="1.1"
            id="Layer_1"
        ...
        <div className="flex justify-between">
          ...
          {isLoading ? (
            <LoadingSkillsGeneration />
          ) : (
            <button
              onClick={generateSkills}
              className="flex gap-x-1 mt-2 bg-gradient-to-r from-[#4E65FF] to-[#6198a0] text-white font-semibold py-2 px-4 rounded-3xl hover:scale-105 transition duration-300"
            >
              <svg
                className="mr-2"
                width="22"
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex justify-center items-center">
                    ...
                    >
                      ...
                      >
                        <path
                          d="M16.7394 3.43362L13.8294 0.523621L8.73938 5.61362L3.64938 0.523621L0.73938 3.43362L5.82938 8.52362L0.73938 13.6136L3.64938 16.5236L8.73938 11.4336L13.8294 16.5236L16.7394 13.6136L11.6494 8.52362L16.7394 3.43362Z"
                          fill="white"
                        />
                      </svg>
                    </button>
